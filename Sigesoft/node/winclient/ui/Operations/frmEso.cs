    ...
    public partial class frmEso : Form
    {
                                ...
                                foreach (ComponentFieldsList f in fieldsByGroupBox)
                                {
                                    ...
                                    switch ((ControlType)f.i_ControlId)
                                    {
                                        ...
                                        case ControlType.ucOSTEOSJL:
                                            var ucOsteto_SJL = new Sigesoft.Node.WinClient.UI.UserControls.ucOsteo_SJ();
                                            ucOsteto_SJL.Name = f.v_ComponentFieldId;
                                            // Establecer evento
                                            ctl = ucOsteto_SJL;
                                            break;
        ...
        private void InitializeData()
        {
            ...
            else
            {
                ...
                chkinterconsulta.Checked = personData.i_SendToTracking == 0 ? false : true; 
        ...
        private void SaveExamWherePendingChange()
        {
            ...
            var acumetria = _tmpServiceComponentsForBuildMenuList
                ...
                var OsteomuscularSJL = _tmpServiceComponentsForBuildMenuList
                       .Find(p => p.v_ComponentId == _componentId)
                       .Fields.Find(p => p.i_ControlId == (int)ControlType.ucOSTEOSJL);

                if (OsteomuscularSJL != null)
                {
                    var UcOsteomuscularSJL = (UserControls.ucOsteo_SJ)FindControlInCurrentTab(OsteomuscularSJL.v_ComponentFieldId)[0];

                    if (UcOsteomuscularSJL.IsChangeValueControl)
                    {
                        _isChangeValue = true;
            ...
            foreach (Control ctrl in ctrlContainer.Controls)
            {
                if (ctrl.Tag != null)
                {
                    ...
                    if (t == typeof(KeyTagControl))
                    {
                        ...
                        else if (keyTagControl.i_ControlId == (int)ControlType.ucOSTEOSJL)
                        {
                            #region Setear valores 

                            dataSourceUserControls = _serviceComponentsInfo.ServiceComponentFields.SelectMany(p => p.ServiceComponentFieldValues).ToList();
                            dataSourceUserControls = dataSourceUserControls.FindAll(p => p.v_ComponentFieldId.Contains("OSJ"));
                            ((UserControls.ucOsteo_SJ)ctrl).DataSource = new List<ServiceComponentFieldValuesList>();
                            ((UserControls.ucOsteo_SJ)ctrl).DataSource = dataSourceUserControls;
                            breakHazChildrenUC = true;
        ...
        private string GetValueControl(int ControlId, Control ctrl)
        {
            ...
            switch ((ControlType)ControlId)
            {
                ...
                case ControlType.UcOsteoMuscular:
                    ...
                    break;
                case ControlType.ucOSTEOSJL:
                    _tmpListValuesOdontograma = ((UserControls.ucOsteo_SJ)ctrl).DataSource;
                    break;
                case ControlType.UcOjoSeco:
                        ...
                        if (ctrl__.Length != 0)
                        {
                            ...
                            switch ((ControlType)cf.i_ControlId)
                            {
                                ...
                                case ControlType.UcOsteoMuscular:
                                    ...
                                    break;
                                case ControlType.ucOSTEOSJL:
                                    ((UserControls.ucOsteo_SJ)ctrl__[0]).ClearValueControl();
                                    break;
                                case ControlType.UcOjoSeco:
                    ...
                    if (field.Length != 0)
                    {
                        ...
                        switch ((ControlType)cf.i_ControlId)
                        {
                            ...
                            case ControlType.UcOsteoMuscular:
                                ...
                                break;
                            case ControlType.ucOSTEOSJL:
                                ((UserControls.ucOsteo_SJ)field[0]).ClearValueControl();
                                break;
                            case ControlType.UcOjoSeco:
        ...
        private void btnGuardarConclusiones_Click(object sender, EventArgs e)
        {
            ...
            if (Result == DialogResult.Yes)
            {
                ...
                serviceDTO.i_SendToTracking = chkinterconsulta.Checked == true ? 1 : 0;
                #region UTILIZAR FIRMA (Suplantar profesional)
        ...
        private void GenerateAnexo7C(string pathFile)
        {
            ...
            var _DataService = _serviceBL.GetServiceReport(_serviceId);
            var filiationData = opacientBL.GetPacientReportEPS(_serviceId);
            var _listMedicoPersonales = _historyBL.GetPersonMedicalHistoryReport(_personId);
            var _listaPatologicosFamiliares = _historyBL.GetFamilyMedicalAntecedentsReport(_personId);
        ...
        private void dptDateGlobalExp_ValueChanged(object sender, EventArgs e)
        {
