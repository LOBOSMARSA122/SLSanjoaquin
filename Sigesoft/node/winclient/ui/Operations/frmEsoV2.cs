...
namespace Sigesoft.Node.WinClient.UI.Operations
{
    public partial class FrmEsoV2 : Form
    {
        ...
        {
            var readingPermission = new ServiceBL().GetRoleNodeComponentProfileByRoleNodeId(int.Parse(Globals.ClientSession.i_CurrentExecutionNodeId.ToString()), int.Parse(Globals.ClientSession.i_RoleId.ToString())).FindAll(p => p.i_Read == 1);
        ...
        private static void SearchControlAndSetValue(Control tlpParent, ServiceComponentList serviceComponentsInfo)
        {
                    ...
                    if (t == typeof(KeyTagControl))
                    {
                        ...
                        switch (keyTagControl.i_ControlId)
                        {
                            ...
                            case (int)ControlType.UcOsteoMuscular:
                                ...
                                breakHazChildrenUc = true;
                                break;
                            case (int)ControlType.ucOSTEOSJL:
                                dataSourceUserControls = serviceComponentsInfo.ServiceComponentFields.SelectMany(p => p.ServiceComponentFieldValues).ToList();
                                dataSourceUserControls = dataSourceUserControls.FindAll(p => p.v_ComponentFieldId.Contains("OSJ"));
                                ((ucOsteo_SJ)ctrl).DataSource = new List<ServiceComponentFieldValuesList>();
                                ((ucOsteo_SJ)ctrl).DataSource = dataSourceUserControls;
                                breakHazChildrenUc = true;
                                break;
                            default:
                    ...
                    if (ctrl.Length != 0)
                    {
                        ...
                        switch ((ControlType)cf.i_ControlId)
                        {
                            ...
                            case ControlType.UcOsteoMuscular:
                                ...
                                break;
                            case ControlType.ucOSTEOSJL:
                                ((ucOsteo_SJ)ctrl[0]).ClearValueControl();
                                break;
        ...
        private Control CreateControlInEso(ComponentFieldsList field, ComponentList component, UltraValidator validator)
        {
            ...
            switch ((ControlType)field.i_ControlId)
            {
                ...
                case ControlType.UcHistorialTriaje:
                    var ucHisorialTriaje = new ucHisorialTriaje {PersonId = _personId};
                    ctl = ucHisorialTriaje;
                    break;

                case ControlType.UcHistorialGrupoSanguineo:
                    var ucHisorialGrupoSanguineo = new ucHistorialGrupoFactor { PersonId = _personId };
                ...
                case ControlType.ucOSTEOSJL:
                    var ucOstetoSjl = new ucOsteo_SJ {Name = field.v_ComponentFieldId};
                    ctl = ucOstetoSjl;
                    break;
                #endregion
                case ControlType.Lista:
        ...
        private UltraTab CrearTab(ComponentList component)
        {
            ...
            //ultraTab.Visible = VisibleTab(component);
        ...
        private void LoadComboBox()
        {
            ...
            var data135 = listDataForCombo.Find(p => p.Id == "135").Items;
            Utils.LoadDropDownList(cbSueño, "Value1", "Id", ConvertClassToStruct(data135), DropDownListAction.Select);
            Utils.LoadDropDownList(cbOrina, "Value1", "Id", ConvertClassToStruct(data135), DropDownListAction.Select);
            Utils.LoadDropDownList(cbDeposiciones, "Value1", "Id", ConvertClassToStruct(data135), DropDownListAction.Select);
            Utils.LoadDropDownList(cbApetito, "Value1", "Id", ConvertClassToStruct(data135), DropDownListAction.Select);
            Utils.LoadDropDownList(cbSed, "Value1", "Id", ConvertClassToStruct(data135), DropDownListAction.Select);
        ...
        private void tcExamList_SelectedTabChanged(object sender, SelectedTabChangedEventArgs e)
        {
            ...
            LoadDataBySelectedComponent(e.Tab.Key, e.Tab.Tag.ToString());
        }
