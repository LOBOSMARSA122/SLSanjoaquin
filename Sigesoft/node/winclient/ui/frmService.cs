...
namespace Sigesoft.Node.WinClient.UI
{
    public partial class frmService : Form
    {
        ...
        private List<ServiceGridJerarquizadaList> GetData(int pintPageIndex, int? pintPageSize, string pstrSortExpression, string pstrFilterExpression)
        {
            ...
            var _objData = _serviceBL.GetServicesPagedAndFiltered_(ref objOperationResult, pintPageIndex, pintPageSize, pstrSortExpression, pstrFilterExpression, pdatBeginDate, pdatEndDate, _componentIds, FCI, FCF, chkIsCampania.Checked ? 1 : 0);
            ...
            //{
                if (saveFileDialog2.ShowDialog() == DialogResult.OK)
                {
                    using (new LoadingClass.PleaseWait(this.Location, "Generando..."))
                    {
                        ...
                        var Audiometriasj = _serviceBL.GetDiagnosticForAudiometria(_serviceId, Constants.AUDIOMETRIA_SJ_ID);
            ...
            using (new LoadingClass.PleaseWait(this.Location, "Generando..."))
            {
                ...
                foreach (var item_0 in ListaServicios)
                {
                    ...
                    foreach (var item in serviceComponents)
                    {
                        ...
                        else if (item.v_ComponentId == Constants.AUDIOMETRIA_SJ_ID)
                        {
                            var ent = serviceComponents.FirstOrDefault(o => o.v_ComponentId == item.v_ComponentId);
                            ent.Orden = 23;
                        }
