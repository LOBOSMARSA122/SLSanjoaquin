    ...
    public partial class frmSchedulePerson : Form
    {
        ...
        private void frmSchedulePerson_Load(object sender, EventArgs e)
        {
            ...
            else if (ModeAgenda == "Edit")
            {
                ...
                dtpDateTimeCalendar.Value = objCalendarList.d_DateTimeCalendar;
        ...
        private void btnSavePacient_Click(object sender, EventArgs e)
        {
            ...
            if (uvPacient.Validate(true, false).IsValid)
            {
                ...
                if (ModePacient == "New")
                {
                    ...
                    objpersonDto.v_Password = txtDocNumber.Text.Trim();
                ...
                if (uvschedule.Validate(true, false).IsValid)
                {
                    ...
                    if (ModeAgenda == "New")
                    {
                        using (new LoadingClass.PleaseWait(this.Location, "Grabando..."))
                        {
                            ...
                            if (ddlMasterServiceId.SelectedValue.ToString() == ((int)MasterService.AtxMedicaParticular).ToString())
                            {
                                CalendarId = _objCalendarBL.AddShedule(ref objOperationResult, objCalendarDto, Globals.ClientSession.GetAsList(), Constants.CONSULTAMEDICA, PacientId, Int32.Parse(ddlMasterServiceId.SelectedValue.ToString()), NuevoContinuacion,0);
                            }
                            else
                            {
                                CalendarId = _objCalendarBL.AddShedule(ref objOperationResult, objCalendarDto, Globals.ClientSession.GetAsList(), _ProtocolId, PacientId, Int32.Parse(ddlMasterServiceId.SelectedValue.ToString()), NuevoContinuacion,0);
                            }
